...
```javascript
 ...
 // 3. 渲染，与 WebGLRenderer 大体流程是一样的，都是 创建renderer,添加renderer.domElement 到 body, 然后渲染，下面看看 render的时候 CSS2DRenderer做了哪些事情。
   ...
   3.2. 将摄像机的视图矩阵转换到 本地坐标系下(这里参考点为原点) 并保存到 _viewMatrix; 将 相对于 本地坐标系下的 投影矩阵 保存到 _viewProjectionMatrix, 两者作用与 _vector有关--标签是否显示
   ...
   3.4.1. 将 object 转换到本地坐标系下的投影空间中，并保存到 _vector, 此时 _vector的坐标 相当于裁剪空间坐标，_vector.z: 深度，_vector.xy：相对于画布中心的坐标(-1~1) -- 
